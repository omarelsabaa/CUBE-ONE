...
int arr[10] = {0, 4, 16, 17, 12, 13, 14, 25, 26, 27};

float id = 0;
//WiFi details
char WIFI_SSID[] = "AndroidAP";
char WIFI_PASSWORD[] = "dgen4102";

//Connection to AWS
char HOST_ADDRESS[] = "a39furswz813o0.iot.us-east-1.amazonaws.com";
char CLIENT_ID[] = "ESP8266-1";
char TOPIC_NAME[] = "$aws/things/ESP8266-1/shadow/update";
...
void setup() {
  ...
  if (ewisp.connect(HOST_ADDRESS, CLIENT_ID) == 0)
  {
    ...
    delay(1000);
    ewisp.publish(TOPIC_2, "Cube 2 Connected to AWS");
...
void loop() {
  ...
  switch (state) {
    case idle: //keep checking for a message. if received then go to state 1
      ...
      if (strcmp(rcvdPayload, "blink") == 0) { //if this message from alexa is received
        Serial.println(" Received blink");
        state = Blink;
      } else if (strcmp(rcvdPayload, "throwcube") == 0) {
        Serial.println("Recieved throwme");
        state = throw_me;
      }
      else {
    ...
    case sending_face_number:
      ...
      //sprintf(payload, dest2); //sending
      //ewisp.publish(TOPIC_2, shadow);
